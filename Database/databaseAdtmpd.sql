...
select sum (montantRemb) as montant, count(*),r.dateRemb,t_pret.versement,t_membre.nom

from t_remboursement r 
--group by montantRemb, --having COUNT(*)>1
inner join t_Pret on r.id_pret=t_Pret.id_pret 
inner join t_membre on t_Pret.membre=t_membre.matricule group by r.montpret.versement,t_membre.nom
GO
select*from t_remboursement
--------------------------Plan de remboursement--------------------
create view Plan_remboursement
as
...
-------------------------------------RAPPORT FICHER DE REMBOURSEMENT-----------------
   ...
   -------------------------------------------------------------
   select t_Pret.versement, sum(montantRemb)as montant  ,COUNT(*) as nobre  from t_remboursement 
inner join t_Pret on t_remboursement.id_pret=t_Pret.id_pret


create view ssituation_epargne as
select nom+' '+postnom as Noms, numeroEpargne, designationEpargne,montantEpargne,dateVersement
from t_epargne
inner join t_membre on t_epargne.matriculeMembre=t_membre.matricule
